  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold">Subjects / Courses</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400">Create and manage subjects/courses</p>
        </div>
        <button @click="openCreate" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-yellow-500 text-gray-900 hover:bg-yellow-600 transition">
          <Plus class="w-4 h-4" /> New Subject
        </button>
      </div>
      ...
      <div class="rounded-xl border p-4 shadow-sm" :class="isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'">
        <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div class="flex flex-col sm:flex-row gap-2">
            <input v-model="filters.q" @keyup.enter="applyFilters" type="text" placeholder="Search by name" class="h-10 rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 text-sm" />
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              <tr v-for="s in items" :key="s.id">
                ...
                <td class="py-3 pr-4">{{ formatPrice(s.price) }}</td>
                <td class="py-3 pr-4 space-x-2">
                  <button @click="openEdit(s)" class="inline-flex items-center gap-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 text-xs">Edit</button>
                  <button @click="confirmDelete(s)" class="inline-flex items-center gap-1 px-2 py-1 rounded border border-red-300 text-red-600 dark:border-red-700 text-xs">Delete</button>
                </td>
              </tr>
              <tr v-if="!loading && (!items || !items.length)">
                <td colspan="4" class="py-6 text-center text-gray-500">No subjects found</td>
              </tr>
        ...
        <div v-if="modal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div class="w-full max-w-lg rounded-xl border shadow-sm" :class="isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'">
            <div class="flex items-center justify-between p-4 border-b" :class="isDark ? 'border-gray-800' : 'border-gray-200'">
              <h3 class="font-semibold">{{ modal.mode === 'create' ? 'New Subject' : 'Edit Subject' }}</h3>
            ...
            <div class="p-4 space-y-3">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="sm:col-span-2">
                  ...
                  <input v-model="form.name" type="text" class="h-10 w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 text-sm" placeholder="e.g. Mathematics" />
                </div>
                <div>
...
const formError = ref('')
const form = ref({ id: null, name: '', price: null })
...
async function loadSubjects() {
  ...
  try {
    ...
    // assuming admin prefix per other endpoints
    const { data } = await api.get('/admin/subjects', { params })
    // normalize Laravel paginator variants
...
function applyFilters() { pagination.value.page = 1; loadSubjects() }
function clearFilters() { filters.value.q = ''; pagination.value.page = 1; loadSubjects() }
function prevPage() { if (pagination.value.page > 1) { pagination.value.page--; loadSubjects() } }
function nextPage() { pagination.value.page++; loadSubjects() }
...
function openEdit(s) {
  ...
  form.value = { id: s.id, name: s.name || '', price: s.price ?? null }
  formError.value = ''
}
...
async function save() {
  ...
  formError.value = ''
  try {
    const payload = { name: form.value.name, price: form.value.price ?? null }
    ...
    if (modal.value.mode === 'create') {
      ...
      await api.patch(`/admin/subjects/${form.value.id}`, payload)
...
function confirmDelete(s) {
  if (!s?.id) return
  const ok = window.confirm(`Delete subject "${s.name}"? This cannot be undone.`)
  if (!ok) return
  deleteSubject(s.id)
}
