    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Create and manage academic years</p>
        </div>
      ...
      <div class="rounded-xl border p-4 shadow-sm" :class="isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'">
        <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div class="flex flex-col sm:flex-row gap-2">
            <input v-model="filters.q" @keyup.enter="applyFilters" type="text" placeholder="Search name" class="h-10 rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 text-sm" />
        ...
        <div class="overflow-x-auto overflow-y-auto max-h-[65vh] sm:max-h-none">
          <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              <tr v-for="y in items" :key="y.id">
                <td class="py-3 pr-4">{{ y.id }}</td>
                <td class="py-3 pr-4 font-medium">{{ y.name }}</td>
                <td class="py-3 pr-4 space-x-2">
                  <button @click="openEdit(y)" class="inline-flex items-center gap-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 text-xs">Edit</button>
                  <button @click="confirmDelete(y)" class="inline-flex items-center gap-1 px-2 py-1 rounded border border-red-300 text-red-600 dark:border-red-700 text-xs">Delete</button>
                </td>
              </tr>
              <tr v-if="!loading && (!items || !items.length)">
                <td colspan="3" class="py-6 text-center text-gray-500">No years found</td>
              </tr>
        ...
        <div v-if="modal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div class="w-full max-w-lg rounded-xl border shadow-sm" :class="isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'">
            <div class="flex items-center justify-between p-4 border-b" :class="isDark ? 'border-gray-800' : 'border-gray-200'">
              <h3 class="font-semibold">{{ modal.mode === 'create' ? 'New Year' : 'Edit Year' }}</h3>
            ...
            <div class="p-4 space-y-3">
              <div>
                ...
                <input v-model="form.name" type="text" class="h-10 w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-3 text-sm" placeholder="e.g. 2024/2025" />
              </div>
...
async function loadYears() {
  ...
  try {
    ...
    const { data } = await api.get('/admin/academic-years', { params })
    ...
    toast.error(e?.response?.data?.message || 'Failed to load years')
  } finally {
    loading.value = false
  }
}

function applyFilters() { pagination.value.page = 1; loadYears() }
function clearFilters() { filters.value.q = ''; pagination.value.page = 1; loadYears() }
function prevPage() { if (pagination.value.page > 1) { pagination.value.page--; loadYears() } }
function nextPage() { pagination.value.page++; loadYears() }

function openCreate() { modal.value = { open: true, mode: 'create' }; form.value = { id: null, name: '' }; formError.value = '' }
function openEdit(y) { modal.value = { open: true, mode: 'edit' }; form.value = { id: y.id, name: y.name || '' }; formError.value = '' }
...
async function save() {
  ...
  try {
    ...
    if (modal.value.mode === 'create') {
      ...
      await api.patch(`/admin/academic-years/${form.value.id}`, payload)
      toast.success('Year updated')
    }
    closeModal(); await loadYears()
  } catch (e) {
...
function confirmDelete(y) {
  if (!y?.id) return
  const ok = window.confirm(`Delete year "${y.name}"? This cannot be undone.`)
  if (!ok) return
  deleteYear(y.id)
}
